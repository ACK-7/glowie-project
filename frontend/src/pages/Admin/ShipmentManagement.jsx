...
const ShipmentManagement = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const safeShipmentsData = Array.isArray(shipmentsData)
        ? shipmentsData
        : [];
      ...
      setError(
        "Failed to load shipments. Please ensure you are logged in as Admin.",
      );
      setShipments([]);
      setTotalShipments(0);
      setTotalPages(1);
  ...
  const filterShipments = () => {
    ...
    if (carrierFilter !== "all") {
      filtered = filtered.filter((shipment) =>
        shipment.carrier_name
          ?.toLowerCase()
          .includes(carrierFilter.toLowerCase()),
      );
    }
  ...
  const handleDeleteShipment = async (shipment) => {
    const confirmed = await showConfirm(
      "Delete Shipment",
      `Permanently delete shipment ${shipment.tracking_number || `#${shipment.id}`}? This cannot be undone.`,
      "warning",
    );
    if (confirmed) {
      try {
          ...
          <div className="bg-[#1a1f28] border border-gray-800 rounded-xl p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-white">
                  {statistics.in_transit_shipments || 0}
                </p>
              </div>
          ...
          <div className="bg-[#1a1f28] border border-gray-800 rounded-xl p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-white">
                  {statistics.delivered_shipments || 0}
                </p>
              </div>
          ...
          <div className="bg-[#1a1f28] border border-gray-800 rounded-xl p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-white">
                  {statistics.delayed_shipments || 0}
                </p>
              </div>
                ...
                filteredShipments.length === 0 ? (
                  ...
                  filteredShipments.map((shipment) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-4">
                        <DropdownMenu
                          items={getDropdownItems(shipment)}
