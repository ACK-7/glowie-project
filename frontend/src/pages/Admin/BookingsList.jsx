...
const BookingsList = () => {
  ...
  const [shipmentBooking, setShipmentBooking] = useState(null);
  ...
  const fetchBookings = useCallback(
    async (showLoader = true, bustCache = false) => {
      ...
      try {
        ...
        if (err.response?.status === 401 || err.response?.status === 403) {
          ...
          showAlert.error(
            "Service Error",
            "Bookings service is currently unavailable.",
          );
        } else {
  ...
  const handleViewBooking = async (booking) => {
    ...
    const loadingAlert = showAlert.loading(
      "Loading...",
  ...
  const handleDeleteBooking = async (booking) => {
    try {
      ...
      if (result) {
        const loadingAlert = showAlert.loading(
          "Deleting...",
          "Please wait while we delete the booking.",
        );
        ...
        try {
          ...
          await showAlert.success(
            "Booking Deleted",
            `Booking ${booking.booking_reference} has been deleted successfully.`,
          );
  ...
  const handleCreateShipment = (booking) => {
    setShipmentBooking(booking);
    setShipmentForm({
      carrier_name: "",
      vessel_name: "",
      container_number: "",
  ...
  const handleSaveShipment = async () => {
    if (!shipmentBooking) return;
    setShipmentSaving(true);
    try {
      await createShipment({
        booking_id: shipmentBooking.id,
        status: "preparing",
        ...shipmentForm,
      });
      setCreateShipmentOpen(false);
      setShipmentBooking(null);
      await showAlert.success(
        "Shipment Created",
        "Shipment created successfully. It will now appear in Tracking Management.",
      );
      fetchBookings(false, true);
    } catch (err) {
      const msg =
      ...
      ? [
          {
            ...
            icon: <FaShip className="text-indigo-400" />,
            onClick: () => handleCreateShipment(booking),
          },
        ]
      : []),
    {
  ...
  const handleBulkAction = async (action) => {
    if (selectedBookings.length === 0) {
      showAlert.warning(
        "No Selection",
        "Please select bookings to perform bulk actions.",
      );
      return;
    }
    ...
    if (result) {
      try {
        ...
        showAlert.error(
          "Bulk Action Failed",
          `Failed to ${action} selected bookings.`,
        );
      }
    }
  };
  ...
  const handleExport = async () => {
    ...
    if (result) {
      try {
        ...
        showAlert.success(
          "Export Started",
          "Your export is being prepared. You will receive a download link shortly.",
        );
      } catch (error) {
        showAlert.error(
          "Export Failed",
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div className="min-w-0 flex-1">
          ...
          <p className="text-gray-400 mt-1">
            Manage and track all vehicle shipping bookings
          </p>
        </div>
              ...
              >
                <thead className="bg-gray-800/50">
                  <tr>
                    ...
                    <th
                      className="px-2 py-3 text-left text-xs font-medium text-gray-300 uppercase"
                      style={{ width: "120px" }}
                    >
                    ...
                    <th
                      className="px-2 py-3 text-center text-xs font-medium text-gray-300 uppercase"
                      style={{ width: "60px" }}
                    >
                    ...
                    bookings.map((booking) => {
                      ...
                      return (
                        <tr key={booking.id} className="hover:bg-gray-800/50">
                          ...
                          <td className="px-2 py-3">
                            <span
                              className={`inline-flex items-center px-1.5 py-0.5 rounded text-xs font-medium ${paymentConfig.bg} ${paymentConfig.text}`}
                            >
                              {paymentConfig.label}
                            </span>
                          </td>
      ...
      {createShipmentOpen && shipmentBooking && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
          <div className="bg-[#1a1f28] border border-gray-700 rounded-xl w-full max-w-lg shadow-2xl">
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-700">
              <div>
                <h2 className="text-lg font-semibold text-white flex items-center gap-2">
                  <FaShip className="text-indigo-400" /> Create Shipment
                </h2>
                <p className="text-xs text-gray-400 mt-0.5">
                  Booking: {shipmentBooking.booking_reference}
                </p>
              </div>
              <button
              ...
              {[
                ...
                <div key={key}>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setShipmentForm((f) => ({ ...f, [key]: e.target.value }))
                    }
                    className="w-full px-3 py-2 bg-gray-800 border border-gray-600 rounded-lg text-gray-200 placeholder-gray-500 focus:ring-2 focus:ring-indigo-500 text-sm"
                  />
                </div>
              ))}
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2 bg-gray-800 border border-gray-600 rounded-lg text-gray-200 focus:ring-2 focus:ring-indigo-500 text-sm"
                  />
                </div>
                <div>
                  <label className="block text-xs font-medium text-gray-300 mb-1">
                  ...
                  <input
                    type="date"
                    value={shipmentForm.estimated_arrival}
                    onChange={(e) =>
                      setShipmentForm((f) => ({
                        ...f,
