...
const DocumentManager = () => {
  ...
  const fetchBookings = async () => {
    try {
      ...
      setError(
        `Failed to load bookings: ${errorMessage}. Please ensure you are logged in as Admin.`,
      );
      setBookings([]);
    } finally {
      setLoading(false);
  ...
  const handleExtractData = async (document) => {
    try {
      ...
      const response = await fetch(
        `http://localhost:8000/api/documents/${document.id}/extract`,
        {
          method: "POST",
          headers: {
      ...
      const extractedData =
        result.data?.extracted_data || result.extracted_data || {};
      const confidenceScore =
        result.data?.confidence_score || result.confidence_score || 0;
      ...
      const errorMessage =
        error.message || "Failed to extract data from document";
  ...
  const handleBulkApprove = async () => {
    ...
    try {
      ...
      showAlert.success(
        "Success!",
        `${selectedDocuments.length} documents approved successfully!`,
      );
  ...
  const handleBulkReject = async () => {
    ...
    try {
      ...
      showAlert.success(
        "Success!",
        `${selectedDocuments.length} documents rejected successfully!`,
      );
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-gray-400">
            Manage shipping documents and compliance verification
          </p>
        </div>
        <button
                  ...
                  <div className="flex flex-col lg:flex-row gap-4">
                    ...
                    <div className="flex gap-3">
                      ...
                      >
                        ...
                        <option value="requires_revision">
                          Needs Revision
                        </option>
                      </select>
              ...
              {!selectedBooking ? (
                ...
                <div className="space-y-3">
                  ...
                  {getFilteredDocuments().map((doc) => (
                    <div
                      key={doc.id}
                      className="bg-gray-800/30 border border-gray-700 rounded-lg p-4 hover:border-gray-600 transition"
                    >
                      <div className="flex items-center gap-4">
                        <input
                          type="checkbox"
            ...
            <div className="bg-[#1a1f28] border border-gray-800 rounded-xl p-6">
              ...
              <div className="grid md:grid-cols-2 gap-3">
                {[
                  ...
                  const isCompleted = documents.some(
                    (doc) =>
                  ...
                  return (
                    <div key={i} className="flex items-center gap-3">
                      ...
                      >
                        {item.label}
                        {isPending && (
                          <span className="text-xs ml-2">(Pending Review)</span>
                        )}
                        {isRejected && (
