...
const GetQuote = () => {
  ...
  const mapOriginCountry = (locationCountry) => {
    ...
    if (
      ...
      return "uk";
    if (c.includes("uae") || c.includes("dubai") || c.includes("united arab"))
      return "uae";
    return "";
  };
  ...
  const handleNlParse = async () => {
    ...
    try {
      const res = await axios.post(
        `${AI_SERVICE_URL}/agents/parse-description`,
      ...
      if (res.data.success && res.data.data) {
        ...
        setNlError(
          'Could not understand that description. Try: "2019 Toyota RAV4 from Japan"',
        );
      }
    } catch (err) {
      ...
      if (err?.code === "ERR_NETWORK" || err?.code === "ECONNREFUSED") {
        setNlError(
          "AI service unavailable. Please fill in the fields manually.",
        );
      } else {
        setNlError(
          `Could not parse description${detail ? ": " + detail : ""}. Try: "2019 Toyota RAV4 from Japan"`,
        );
      }
    } finally {
      setNlLoading(false);
  ...
  const triggerSuggestion = useCallback((make, model) => {
    ...
    validationTimerRef.current = setTimeout(async () => {
      ...
      try {
        const res = await axios.post(
          `${AI_SERVICE_URL}/agents/suggest-vehicle`,
          { make, model },
        );
        if (res.data.success && res.data.data) {
  ...
  const triggerValidation = useCallback((data) => {
    ...
    previewTimerRef.current = setTimeout(async () => {
      try {
        const res = await axios.post(
          `${AI_SERVICE_URL}/agents/validate-vehicle`,
          data,
        );
      ...
      <section className="bg-gradient-to-r from-blue-900 via-blue-800 to-blue-900 text-white py-20">
        <div className="container-custom text-center">
          ...
          <p className="text-xl text-blue-100 max-w-3xl mx-auto">
            Fill in your details and get an instant, AI-powered transparent
            quote for shipping your car to Uganda.
          </p>
        </div>
      </section>
        ...
        <div className="container-custom max-w-4xl mx-auto">
          ...
          {prefilledCar && (
            <div className="mb-8 bg-blue-50 border border-blue-200 rounded-xl p-4 flex items-center gap-4">
              {prefilledCar.image && (
                <img
                  src={prefilledCar.image}
                  alt={prefilledCar.name}
              ...
              <div className="flex-1">
                <p className="text-blue-800 font-semibold text-sm">
                  ✅ Vehicle details pre-filled from:{" "}
                  <span className="font-bold">{prefilledCar.name}</span>
                </p>
                <p className="text-blue-600 text-xs mt-0.5">
                  Review the fields below and adjust anything that needs
                  changing.
                </p>
              </div>
              <button
              ...
              {currentStep === 1 && (
                <div className="space-y-6">
                  <h2 className="text-2xl font-bold text-navy-900 mb-6">
                    ...
                    {prefilledCar && (
                      <span className="ml-3 text-sm font-normal text-blue-500 bg-blue-50 px-2 py-1 rounded-full">
                        ✨ Pre-filled
                      </span>
                    )}
                  </h2>
                  ...
                  <div className="bg-gradient-to-r from-purple-50 to-blue-50 border border-purple-200 rounded-xl p-4">
                    <label className="block text-sm font-semibold text-purple-800 mb-2">
                      🤖 Describe your car in plain text{" "}
                    ...
                    <div className="bg-amber-50 border border-amber-200 rounded-lg px-4 py-3 flex items-start gap-3">
                      ...
                      <div className="flex-1 text-sm">
                        ...
                        <span className="text-amber-700">
                          {formData.make} {formData.model} is typically a{" "}
                          <strong>{suggestion.vehicleType}</strong>
                          {suggestion.engineSize
                            ? `, ~${suggestion.engineSize}cc engine`
                            : ""}
                          {suggestion.originCountry
                            ? `, commonly imported from ${suggestion.originCountry}`
                            : ""}
                          .
                        </span>
                        <button
                          ...
                          onClick={() => {
                            setFormData((prev) => ({
                              ...
                              ...(suggestion.engineSize &&
                                !prev.engineSize && {
                                  engineSize: suggestion.engineSize,
                                }),
                              ...(suggestion.originCountry &&
                                !prev.originCountry && {
                  ...
                  {suggestionLoading && (
                    <div className="text-xs text-gray-400 flex items-center gap-2">
                      <FaSpinner className="animate-spin" /> AI is looking up{" "}
              ...
              {currentStep === 2 && (
                <div className="space-y-6">
                  <h2 className="text-2xl font-bold text-navy-900 mb-6">
                    ...
                    {prefilledCar && formData.originCountry && (
                      <span className="ml-3 text-sm font-normal text-blue-500 bg-blue-50 px-2 py-1 rounded-full">
                        ✨ Pre-filled
                      </span>
                    )}
                          ...
                          <div className="flex items-start gap-3">
                            ...
                            <div className="flex-1">
                              ...
                              <p className="text-sm text-gray-600">
                                Most economical. Vehicle driven onto ship deck.
                              </p>
                            </div>
                          </div>
                        </label>
                        ...
                        >
                          ...
                          <div className="flex items-start gap-3">
                            ...
                            <div className="flex-1">
                              ...
                              <p className="text-sm text-gray-600">
                                Maximum protection. Private enclosed container.
                              </p>
                            </div>
                          </div>
                        </label>
                  ...
                  {(previewLoading || livePreview) && (
                    <div className="mt-4 bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-200 rounded-xl p-5">
                      <div className="flex items-center gap-2 mb-3">
                        ...
                        {previewLoading && (
                          <FaSpinner className="animate-spin text-blue-400 ml-auto" />
                        )}
                      </div>
                        ...
                        <>
                          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-center">
                            <div className="bg-white rounded-lg p-3 shadow-sm">
                              ...
                              <p className="font-bold text-gray-900">
                                $
                                {(
                                  livePreview.breakdown?.shipping || 0
                                ).toLocaleString()}
                              </p>
                            </div>
                            <div className="bg-white rounded-lg p-3 shadow-sm">
                              ...
                              <p className="font-bold text-gray-900">
                                $
                                {(
                                  livePreview.breakdown?.customs_duty || 0
                                ).toLocaleString()}
                              </p>
                            </div>
                            <div className="bg-white rounded-lg p-3 shadow-sm">
                              ...
                              <p className="font-bold text-gray-900">
                                $
                                {(
                                  (livePreview.breakdown?.vat || 0) +
                                  (livePreview.breakdown?.levies || 0)
                                ).toLocaleString()}
                              </p>
                            </div>
                            <div className="bg-blue-600 rounded-lg p-3 shadow-sm">
                              ...
                              <p className="font-bold text-white text-lg">
                                $
                                {(livePreview.total_cost || 0).toLocaleString()}
                              </p>
                            </div>
                          </div>
                          {livePreview.estimated_delivery_days && (
                            <p className="text-xs text-blue-600 mt-2 text-center">
                              🚢 Estimated delivery:{" "}
                              <strong>
                                {livePreview.estimated_delivery_days} days
                              </strong>{" "}
                              to Uganda
                            </p>
                          )}
                ...
                <div className="space-y-6">
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              <div className="flex-1 max-w-xs bg-gray-200 rounded-full h-2">
                                <div
                                  ...
                                  style={{
                                    width: `${(estimatedQuote.confidence_score * 100).toFixed(0)}%`,
                                  }}
                                ></div>
                              </div>
                              <span className="text-xs font-bold text-purple-700">
                                {(
                                  estimatedQuote.confidence_score * 100
                                ).toFixed(0)}
                                %
                              </span>
                            </div>
                  ...
                  <div className="bg-gray-50 rounded-xl p-6 space-y-4">
                    <div className="flex justify-between items-center pb-3 border-b border-gray-200">
                      <span className="text-gray-700">
                        Ocean Freight ({formData.shippingMethod.toUpperCase()})
                      </span>
                      <span className="font-semibold">
                        ${estimatedQuote.shipping.toLocaleString()}
                      </span>
                    </div>
                    <div className="flex justify-between items-center pb-3 border-b border-gray-200">
                      <span className="text-gray-700">
                      ...
                      <span className="font-semibold">
                        ${estimatedQuote.customs.toLocaleString()}
                      </span>
                    </div>
                    <div className="flex justify-between items-center pb-3 border-b border-gray-200">
                      <span className="text-gray-700">VAT (18%)</span>
                      <span className="font-semibold">
                        ${estimatedQuote.vat.toLocaleString()}
                      </span>
                    </div>
                    <div className="flex justify-between items-center pb-3 border-b border-gray-200">
                      <span className="text-gray-700">Levies & Fees</span>
                      <span className="font-semibold">
                        ${estimatedQuote.levies.toLocaleString()}
                      </span>
                    </div>
                    <div className="flex justify-between items-center pt-3">
                      ...
                      <span className="text-3xl font-bold text-blue-600">
                        ${parseFloat(estimatedQuote.total).toLocaleString()}
                      </span>
                    </div>
                  </div>
                    ...
                    <div className="flex items-start gap-4">
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-sm text-green-800 mb-3">
                          We've sent login credentials to{" "}
                          <strong>{formData.email}</strong>. Check your email to
                          access your customer portal where you can:
                        </p>
                        ...
                        <div className="mt-4 pt-4 border-t border-green-200">
                          <p className="text-xs text-green-700">
                            <strong>📧 Check your email</strong> for your
                            temporary password and portal access link.
                          </p>
                        </div>
                      </div>
                    </div>
                  ...
                  <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mt-6">
                    <p className="text-sm text-gray-700">
                      <strong>Note:</strong> This is a preliminary estimate
                      saved with reference{" "}
                      <strong>{estimatedQuote.reference}</strong>. A member of
                      our team will review and approve your quote. You'll be
                      notified via email when it's ready to accept.
                    </p>
                  </div>
        ...
        <div className="container-custom max-w-5xl mx-auto">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 text-center">
            ...
            <div>
              ...
              <p className="text-gray-600 text-sm">
                Intelligent pricing with market analysis
              </p>
            </div>
            <div>
              <div className="text-4xl mb-3">🎯</div>
