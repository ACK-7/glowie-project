...
const CustomerPortal = () => {
  ...
  const loadAllCustomerData = async () => {
    ...
    try {
      ...
      const extractedBookings =
        bookingsData?.data?.data || bookingsData?.data || [];
  ...
  const confirmQuote = async (quoteId) => {
    ...
    try {
      ...
      showAlert.error(
        "Confirmation Failed",
        error.response?.data?.message || "Failed to confirm quote",
      );
    } finally {
      setIsLoading(false);
    }
  ...
  const handleDocumentUploadSuccess = () => {
    ...
    customerService
      ...
      .then((data) => {
        const safeDocuments = Array.isArray(data.data)
          ? data.data
          : Array.isArray(data)
            ? data
            : [];
  ...
  const handleSaveProfile = async () => {
    ...
    try {
      ...
      const response =
        await customerService.updateCustomerProfile(editFormData);
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                          ...
                          <p className="text-lg font-semibold text-gray-900">
                            {customerProfile?.first_name || "N/A"}
                          </p>
                        </div>
                        <div>
                          <label className="block text-sm font-medium text-gray-700 mb-1">
              ...
              {isLoading ? (
                ...
                <div className="text-center py-8">
                  ...
                  <p className="text-sm text-gray-500 mt-2">
                    Your confirmed quotes will appear here as bookings.
                  </p>
                </div>
              ) : (
                <div className="space-y-6">
                        ...
                        {(booking.status === 'delivered' || booking.status === 'completed') && (
                          <div className="mt-3">
                            <button
                              ...
                              className="flex items-center gap-2 text-sm text-yellow-600 hover:text-yellow-700 font-medium bg-yellow-50 hover:bg-yellow-100 px-3 py-1.5 rounded-lg transition-colors"
                            >
                        ...
                        <div className="flex justify-between items-start mb-4">
                          <div>
                            <h3 className="text-lg font-bold text-gray-900">
                              {quote.vehicle_details?.make || "Vehicle"}{" "}
                              {quote.vehicle_details?.model || ""}{" "}
                              {quote.vehicle_details?.year
                                ? `(${quote.vehicle_details.year})`
                                : ""}
                            </h3>
                            <p className="text-gray-600">
                            ...
                            <p className="text-sm text-gray-500">
                              {quote.route?.origin_city || "Origin"},{" "}
                              {quote.route?.origin_country || ""} →{" "}
                              {quote.route?.destination_city || "Destination"}
                            </p>
                          </div>
            ...
            <div className="bg-white rounded-xl shadow-md p-8">
              ...
              {isLoading ? (
                ...
                <div className="text-center py-8">
                  ...
                  <p className="text-sm text-gray-500 mt-2">
                    Your confirmed bookings will appear here for tracking.
                  </p>
                </div>
              ) : (
                <div className="space-y-6">
                  ...
                  {selectedBooking ? (
                    ...
                    <div className="text-center py-12">
                      ...
                      <p className="text-gray-600">
                        Select a shipment above to view tracking details
                      </p>
                    </div>
                  )}
                </div>
            ...
            <div className="bg-white rounded-xl shadow-md p-8">
              ...
              {!Array.isArray(bookings) || bookings.length === 0 ? (
                <div className="text-center py-8">
                  ...
                  <p className="text-sm text-gray-500 mt-2">
                    Contracts will appear here when you confirm bookings.
                  </p>
                </div>
              ) : (
                <div className="space-y-4">
                  {Array.isArray(bookings) &&
                    bookings.map((booking) => (
                      <div
                        key={booking.id}
                        className="border-l-4 border-green-500 bg-green-50 p-6 mb-6"
                      >
                    ...
                    <>
                      <div className="flex justify-between py-3 border-b">
                        ...
                        <span className="font-semibold">
                          {bookings[0].vehicle_make} {bookings[0].vehicle_model}{" "}
                          ({bookings[0].vehicle_year})
                        </span>
                      </div>
                      <div className="flex justify-between py-3 border-b">
                        <span className="text-gray-700">Route</span>
                        <span className="font-semibold">
                          {bookings[0].origin_city},{" "}
                          {bookings[0].origin_country} →{" "}
                          {bookings[0].destination_city}
                        </span>
                      </div>
                      <div className="flex justify-between py-3 border-b">
                        <span className="text-gray-700">Status</span>
                        <span className="font-semibold">
                          {bookings[0].status || "Processing"}
                        </span>
                      </div>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-xl shadow-md p-8">
              ...
              <p className="text-gray-600 mb-6">
                Upload your personal documents for customs clearance and
                verification
              </p>
            ...
            <div className="bg-white rounded-xl shadow-md p-8">
              ...
              {isLoading ? (
                ...
                <div className="text-center py-8">
                  ...
                  <p className="text-sm text-gray-500 mt-2">
                    Upload documents using the section above.
                  </p>
                </div>
              ) : (
                <div className="space-y-4">
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            ...
                            <p className="text-sm text-gray-600">
                              Type: {document.document_type} • Uploaded:{" "}
                              {customerService.formatDate(document.created_at)}
                            </p>
                        ...
                        <div className="flex items-center gap-3">
                          <span
                            className={`px-3 py-1 rounded-full text-sm font-medium ${customerService.getStatusColor(
                              document.status,
                            )}`}
                          >
            ...
            <div className="bg-white rounded-xl shadow-md p-8">
                    ...
                    >
                      <div className="flex justify-between items-center">
                        <div>
                          ...
                          <p className="text-sm text-gray-600">
                            Date:{" "}
                            {customerService.formatDate(payment.created_at)}
                          </p>
                          {payment.booking && (
                            <p className="text-sm text-gray-600">
                        ...
                        <div className="text-right">
                          ...
                          <span
                            className={`inline-block px-3 py-1 rounded-full text-sm font-medium mt-2 ${customerService.getStatusColor(
                              payment.status,
                            )}`}
                          >
              ...
              <div className="mt-6 p-4 bg-blue-50 rounded-lg">
                <p className="text-sm text-gray-700">
                  ...
                  You can pay via bank transfer, mobile money, or credit card.
                </p>
              </div>
            </div>
          </div>
