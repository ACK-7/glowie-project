...
const Header = () => {
  ...
  return (
    ...
    >
          ...
          <div className="flex items-center gap-6">
            ...
            >
              ...
              <span className="hidden md:inline">
                Hot Line: +256 756 689882
              </span>
            </a>
            <a
              href="mailto:info@shipwithglowie.com"
              className="flex items-center gap-2 hover:text-gold-400 transition"
            >
      ...
      {/* Main Navigation */}
      <div
        className={`container-custom flex justify-between items-center py-4 transition-all duration-300 ${
          isHomePage
            ? isScrolled
              ? "bg-white/95 backdrop-blur-md"
              : "bg-transparent"
      ...
      >
        ...
        <Link to="/" className="flex items-center gap-2">
          <div className="text-2xl md:text-3xl font-heading font-bold">
            <span
              className={`${isHomePage && !isScrolled ? "text-white" : "gradient-text"}`}
            >
              ShipWith
            </span>
            <span
              className={`${isHomePage && !isScrolled ? "text-yellow-400" : "text-navy-900"}`}
            >
              Glowie
            </span>
          </div>
        ...
        <nav className="hidden lg:flex items-center space-x-8">
          ...
          >
            ...
            >
              About Us
              <FaChevronDown
                className={`text-xs transition-transform duration-300 ${isAboutOpen ? "rotate-180" : ""}`}
              />
              <span
                className={`absolute bottom-0 left-0 w-0 h-0.5 group-hover:w-full transition-all duration-300 ${
                  isHomePage && !isScrolled
        ...
        <div className="flex items-center gap-3">
          {!authLoading && isCustomerLoggedIn && <NotificationBell />}
          <Link
            to="/login"
          ...
          <Link
            to="/track"
            className={`hidden md:flex items-center gap-2 px-4 py-2 rounded-md font-medium transition-all duration-300 shadow-sm hover:shadow-md ${
              isHomePage && !isScrolled
